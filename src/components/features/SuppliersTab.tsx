  ...
  const columns: Column<any>[] = [
    {
      ...
      accessor: (row) => (
        <div>
          ...
          {row.contactPerson && <span className="text-[10px] text-slate-500">{row.contactPerson}</span>}
        </div>
      ),
      sortKey: 'name',
    ...
    },
    {
      header: 'GSTIN',
      accessor: (row) => (
        <span className="text-slate-300 font-mono text-[11px]">{row.gstin || '—'}</span>
      ),
    ...
    },
    {
      header: 'Credit Days',
      accessor: (row) => (
        <span className="text-slate-300">{row.creditDays ?? '—'}</span>
      ),
    ...
    },
    {
      header: 'Balance',
      accessor: (row) => (
        <span className={`font-bold ${(row.outstandingBalance || 0) > 0 ? 'text-amber-400' : 'text-slate-400'}`}>
      ...
      accessor: (row) => (
        <div className="flex items-center gap-1.5">
          <button
            ...
            className="p-1.5 rounded-lg hover:bg-teal-500/10 text-slate-400 hover:text-teal-400 transition-colors cursor-pointer"
            title="Edit"
          >
            <FiEdit size={14} />
          </button>
        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <p className="text-xs text-slate-500">Supplier Details</p>
            </div>
          </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-5">
          ...
          <div className="bg-slate-900/60 border border-slate-850 rounded-xl p-5 space-y-4">
            <h3 className="text-xs font-bold text-slate-400 uppercase tracking-wider flex items-center gap-2">
              <FiUser size={14} className="text-teal-400" /> Contact Information
            </h3>
            <div className="grid grid-cols-2 gap-4">
              <div>
                <span className="text-[10px] text-slate-500 uppercase font-bold block mb-0.5">Contact Person</span>
                <span className="text-sm text-slate-200">{s.contactPerson || '—'}</span>
              </div>
              <div>
                <span className="text-[10px] text-slate-500 uppercase font-bold block mb-0.5">Phone</span>
                <span className="text-sm text-slate-200 flex items-center gap-1.5">
                  {s.phone ? <><FiPhone size={12} className="text-teal-400" />{s.phone}</> : '—'}
                </span>
              </div>
              <div>
                <span className="text-[10px] text-slate-500 uppercase font-bold block mb-0.5">Email</span>
                <span className="text-sm text-slate-200 flex items-center gap-1.5">
                  {s.email ? <><FiMail size={12} className="text-teal-400" />{s.email}</> : '—'}
                </span>
              </div>
              <div>
                <span className="text-[10px] text-slate-500 uppercase font-bold block mb-0.5">Status</span>
          ...
          <div className="bg-slate-900/60 border border-slate-850 rounded-xl p-5 space-y-4">
            <h3 className="text-xs font-bold text-slate-400 uppercase tracking-wider flex items-center gap-2">
              <FiCreditCard size={14} className="text-teal-400" /> Business Details
            </h3>
            <div className="grid grid-cols-2 gap-4">
              <div>
                <span className="text-[10px] text-slate-500 uppercase font-bold block mb-0.5">GSTIN</span>
                <span className="text-sm text-slate-200 font-mono">{s.gstin || '—'}</span>
              </div>
              <div>
                <span className="text-[10px] text-slate-500 uppercase font-bold block mb-0.5">Credit Days</span>
                <span className="text-sm text-slate-200">{s.creditDays ?? '—'} days</span>
              </div>
              <div>
                <span className="text-[10px] text-slate-500 uppercase font-bold block mb-0.5">Opening Balance</span>
                <span className="text-sm text-slate-200">₹{(s.openingBalance || 0).toFixed(2)}</span>
              </div>
              <div>
                <span className="text-[10px] text-slate-500 uppercase font-bold block mb-0.5">Outstanding</span>
          ...
          <div className="bg-slate-900/60 border border-slate-850 rounded-xl p-5 space-y-4">
            <h3 className="text-xs font-bold text-slate-400 uppercase tracking-wider flex items-center gap-2">
              <FiMapPin size={14} className="text-teal-400" /> Address
            </h3>
            <div className="grid grid-cols-2 gap-4">
              <div className="col-span-2">
                <span className="text-[10px] text-slate-500 uppercase font-bold block mb-0.5">Street Address</span>
                <span className="text-sm text-slate-200">{s.address || '—'}</span>
              </div>
              <div>
                <span className="text-[10px] text-slate-500 uppercase font-bold block mb-0.5">City</span>
                <span className="text-sm text-slate-200">{s.city || '—'}</span>
              </div>
              <div>
                <span className="text-[10px] text-slate-500 uppercase font-bold block mb-0.5">State</span>
                <span className="text-sm text-slate-200">{s.state || '—'}</span>
              </div>
              <div>
                <span className="text-[10px] text-slate-500 uppercase font-bold block mb-0.5">Pincode</span>
                <span className="text-sm text-slate-200">{s.pincode || '—'}</span>
              </div>
            </div>
          </div>
          ...
          <div className="bg-slate-900/60 border border-slate-850 rounded-xl p-5 space-y-4">
            <h3 className="text-xs font-bold text-slate-400 uppercase tracking-wider flex items-center gap-2">
              <FiFileText size={14} className="text-teal-400" /> Notes
            </h3>
            <p className="text-sm text-slate-300 whitespace-pre-wrap">{s.notes || 'No notes added.'}</p>
          </div>
        </div>
      </div>
    );
    ...
    <div className="space-y-6 animate-fadeIn">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-xl font-bold text-white tracking-tight flex items-center gap-2.5">
            <FiTruck className="text-teal-400" size={22} />
            Supplier Management
          </h1>
      ...
      <div className="grid grid-cols-4 gap-4">
        {[
          ...
          { label: 'With GSTIN', value: stats.withGstin, color: 'text-teal-400' },
        ].map((stat, i) => (
          <div key={i} className="bg-slate-900/60 border border-slate-850 rounded-xl p-4 text-center">
      ...
      >
        <div className="space-y-5">
          ...
          <div className="space-y-3">
            <h4 className="text-[10px] font-bold text-slate-500 uppercase tracking-wider">Basic Information</h4>
            <div className="grid grid-cols-2 gap-3">
              <div className="col-span-2">
                ...
                <input
                  ...
                  className="w-full px-3 py-2 bg-slate-955 border border-slate-800 rounded-lg text-slate-200 text-xs focus:border-teal-500 focus:ring-1 focus:ring-teal-500 outline-none"
                />
              </div>
              <div>
                ...
                <input
                  ...
                  maxLength={15}
                  className="w-full px-3 py-2 bg-slate-955 border border-slate-800 rounded-lg text-slate-200 text-xs font-mono focus:border-teal-500 focus:ring-1 focus:ring-teal-500 outline-none"
                />
              </div>
              <div>
          ...
          {/* Contact */}
          <div className="space-y-3">
            <h4 className="text-[10px] font-bold text-slate-500 uppercase tracking-wider">Contact Details</h4>
            <div className="grid grid-cols-2 gap-3">
              <div>
          ...
          {/* Address */}
          <div className="space-y-3">
            <h4 className="text-[10px] font-bold text-slate-500 uppercase tracking-wider">Address</h4>
          ...
          {/* Financial */}
          <div className="space-y-3">
            <h4 className="text-[10px] font-bold text-slate-500 uppercase tracking-wider">Financial</h4>
            <div className="grid grid-cols-2 gap-3">
              <div>
                <label className="block text-[10px] font-bold text-slate-500 uppercase mb-1">Credit Days</label>
                <input
                  type="number"
              ...
              <div>
                <label className="block text-[10px] font-bold text-slate-500 uppercase mb-1">Opening Balance (₹)</label>
                <input
                  type="number"
          ...
          <div>
            ...
            <textarea
              ...
              className="w-full px-3 py-2 bg-slate-955 border border-slate-800 rounded-lg text-slate-200 text-xs focus:border-teal-500 focus:ring-1 focus:ring-teal-500 outline-none resize-none"
            />
          </div>
        </div>
      </Modal>
