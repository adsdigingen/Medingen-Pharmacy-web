...
export const HistoryTab: React.FC<HistoryTabProps> = ({
  ...
  const columns: Column<any>[] = [
    {
      header: 'Invoice #',
      accessor: (row) => <span className="font-mono font-bold text-teal-400">{row.billNumber}</span>,
    ...
    },
    {
      ...
      accessor: (row) => (
        <div>
          <span className="font-bold text-slate-200 block">{row.customer?.name || 'Walk-in Customer'}</span>
          {row.customer?.mobile && <span className="text-[10px] text-slate-500 font-mono block">Mob: {row.customer.mobile}</span>}
        </div>
      ),
    ...
    {
      header: 'Net Total',
      accessor: (row) => <span className="font-bold font-mono text-slate-100">₹{(row.netAmount || 0).toFixed(2)}</span>,
    ...
    },
    {
      ...
      accessor: (row) => (
        <div className="flex gap-2 justify-end font-bold text-[10px]">
          <button onClick={() => setInvoiceDetail(row)} className="text-teal-400 hover:underline cursor-pointer">View</button>
          <button onClick={() => handleFetchReceipt(row.id)} className="text-slate-350 hover:underline cursor-pointer">Receipt</button>
          {row.status === 'COMPLETED' && (
            <>
              <button onClick={() => { setInvoiceDetail(row); setIsCancelModalOpen(true); }} className="text-rose-455 hover:underline cursor-pointer">Cancel</button>
  ...
  return (
    <div className="space-y-6 animate-fadeIn font-sans text-xs text-slate-400">
      ...
      {/* Title */}
      <div>
        <h2 className="text-base font-bold text-white uppercase tracking-wider">Invoice Registers</h2>
        <p className="text-[11px] text-slate-500 font-medium">Verify daily checkouts, print duplicate thermal receipts, or process customer returns</p>
      </div>

      {/* Filters */}
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 bg-slate-900/20 border border-slate-900 p-4 rounded-2xl text-xs shadow-lg items-center">
        <div className="sm:col-span-2 relative">
          <input
            ...
            className="w-full pl-8 pr-3 py-1.5 rounded-lg bg-slate-955 border border-slate-800 text-slate-200 focus:outline-none"
          />
          <svg className="absolute left-2.5 top-2.5 h-4 w-4 text-slate-500" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2.5}>
        ...
        <div>
          <select
            ...
            className="w-full px-3 py-1.5 rounded-lg bg-slate-955 border border-slate-800 text-slate-250 focus:outline-none"
          >
      ...
      {invoiceLoading ? (
        ...
        <div className="border border-slate-900 rounded-2xl overflow-hidden shadow-lg bg-slate-900/10">
          ...
          {invoiceTotal > invoiceLimit && (
            <div className="flex items-center justify-between px-6 py-4 bg-slate-950/40 border-t border-slate-900 text-xs">
              <span className="text-slate-500">Total {invoiceTotal} invoices</span>
              <div className="flex gap-2">
                <button 
                  onClick={() => setInvoicePage(p => Math.max(1, p - 1))} 
                  disabled={invoicePage === 1} 
                  className="px-3.5 py-1.5 bg-slate-955 hover:bg-slate-850 border border-slate-800 rounded-lg text-slate-300 disabled:opacity-40 disabled:pointer-events-none transition-all cursor-pointer font-bold"
                >
                  Prev
                </button>
                <span className="px-3.5 py-1.5 font-bold font-mono text-slate-350 bg-slate-955 rounded-lg flex items-center border border-slate-850">
